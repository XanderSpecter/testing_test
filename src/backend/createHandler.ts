 ...
 * @param handler Обработчик данных запроса - на вход принимает `params` - провалидированные параметры из `query` и/или `body` запроса. Внутри можно сделать логику или обращения к БД. Должен вернуть данные или хотя бы не сломаться для достижения успеха.
